...
# Inicialización del proceso aleatorio X(t):
A = [-3, -1, 1, 3]            # Valores para la amplitud
N =  16                       # Hay 16 posibilidades de la onda por el módulo utilizado
Todo = np.empty((N, len(t)))    # N funciones del tiempo x(t) con T puntos

# Para la señal se aplica cada valor de amplitud (-1, -3, 1, 3)
for i in A:
    B = i*np.cos(2*np.pi*fc*t) + i*np.sin(2*np.pi*fc*t)
    C = -i*np.cos(2*np.pi*fc*t) + i*np.sin(2*np.pi*fc*t)
    D = i*np.cos(2*np.pi*fc*t) + -i*np.sin(2*np.pi*fc*t)
    E = -i*np.cos(2*np.pi*fc*t) + -i*np.sin(2*np.pi*fc*t)
    Todo[i,:]   = B
    Todo[i+1,:] = C
    Todo[i+2,:] = D
    Todo[i+3,:] = E
...
P = [np.mean(Todo[:,i]) for i in range(len(t))]
...
for n in range(N):
    for i, tau in enumerate(desplazamiento):
        corr[n, i] = np.correlate(Todo[n,:], np.roll(Todo[n,:], tau))/T
